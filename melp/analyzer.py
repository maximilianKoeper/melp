...
class TileAnalyzer:
    ...
    def addTileHits(self, filename):
        ...
        if ttree_mu3e.run in self.Detector.AddedRuns:
            print("ERROR: RUN ", ttree_mu3e.run, " already loaded into Detector")
            return
        else:
            self.Detector.AddedRuns.append(ttree_mu3e.run)
            pass
        ...
        for frame in range(ttree_mu3e.GetEntries()):
            ...
            for i in range(len(ttree_mu3e.tilehit_tile)):
                ...
                # tilehit = Hit(edep = edep, mc_i = mc_i, hid = hid, impact_vec = p_xyz)
                self.Detector.TileDetector.addHit(tile, tilehit)
    ...
    def getHitRate(self, tileID=-1):
        hitrate = [[0], [0], [0], [0], [0], [0]]
        if tileID == -1:
            for tile in self.Detector.TileDetector.tile:
                hitrate[0].append(self.Detector.TileDetector.getPos(tile)[2])
                hitrate[1].append(0)
                hitrate[2].append(0)
                hitrate[3].append(0)
                hitrate[4].append(0)
                ...
                for hit in self.Detector.TileDetector.tile[tile].hits:
            ...
            hitrate[0][0] = self.Detector.TileDetector.tile[tileID].pos[2]  # z position

            for hit in self.Detector.TileDetector.tile[tileID].hits:
        ...
        self.Detector.TileDetector.addRateResult(hitrate)
        return hitrate
    ...
    def getHitAngle(self, tileID=-1, rec_type="Truth", hit_type="primary", angle="phi"):
        ...
        for tileID in self.Detector.TileDetector.tile:
            for hit in self.Detector.TileDetector.tile[tileID].hits:
                ...
                ##############
                if angle == "norm":
                    hitangle[1].append(mf.angle_between(hit.impact_vec, self.Detector.TileDetector.tile[tileID].dir))
                elif angle == "theta":
                    hitangle[1].append(mf.angle_between(hit.impact_vec, np.array([0, 0, -1])))
                elif angle == "phi":
                    vector = -np.array(self.Detector.TileDetector.tile[tileID].dir)
                    hitangle[1].append(-mf.angle_between_phi(hit.impact_vec[0:2], vector[0:2]))
                else:
                    raise ValueError('ERROR: angle != [norm, theta, phi]')

                hitangle[0].append(self.Detector.TileDetector.tile[tileID].pos[2])

        self.Detector.TileDetector.addAngleResult(hitangle)
        return hitangle
